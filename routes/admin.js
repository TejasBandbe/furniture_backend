...
adminRouter.post('/register', async(request, response) => {
    try{
        ...
        statement = `insert into users values(default, '${firstName}', '${lastName}', '${emailId}', 
...
adminRouter.post('/login', async(request, response) => {
    try{
        ...
        var statement = `select id, password from users where emailId = '${emailId}'`;
        var data = await executeQuery(statement);
        if(data.length === 0){
        ...
        if(!isPasswordValid){
            ...
            response.status(200).send({"message": "logged in successfully", "adminId": data[0].id});
        }
    }catch(error){
        response.status(400).send({"error": error});
    }
...
adminRouter.post('/updatePassword', async(request, response) => {
    try{
        ...
        var statement = `select password from users where id = ${adminId}`;
        var data = await executeQuery(statement);
        ...
        const isPasswordValid = await bcrypt.compare(oldPassword, data[0].password);
        console.log(isPasswordValid);
        if(!isPasswordValid){
            response.status.send({"error": "old password is wrong"});
            return;
        }
        else{
...
adminRouter.post('/updateStaticContentById', async(request, response) => {
    try{
        ...
        var statement = `update staticcontent set contentValue='${contentValue}' where id = ${contentId}`;
        var data = await executeQuery(statement);

        if(data.affectedRows === 0){
            response.status(200).send({"error": "error occured while updating shop name"});
            return;
        }
        else{
...
adminRouter.post('/addFolder', async(request, response) => {
    try{
        ...
        var statement = `insert into folders(folderName) values('${folderName}')`;
        var data = await executeQuery(statement);
